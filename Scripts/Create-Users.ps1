...
Write-Host "  Safe Users: $SafeUsers" -ForegroundColor Green
Write-Host "  Risk Users: $RiskUsers" -ForegroundColor Yellow
...
try {
    ...
    foreach ($User in $Users) {
        ...
        # 🟡 HIGH RISKS
        elseif ($User.BadPasswordCount -ge 3 -and $User.BadPasswordCount -lt 8 -and $User.LastLogonDate -ne "") {
            try {
        ...
        elseif ($User.SamAccountName -like "svc_*" -and $User.Enabled -eq "TRUE" -and $User.LastLogonDate -ne "") {
            try {
                $LastLogonDate = [DateTime]::ParseExact($User.LastLogonDate, "M/d/yyyy h:mm tt", $null)
        ...
        elseif ($User.LogonCount -ge 500 -and $User.LastLogonDate -ne "") {
            try {
                $LastLogonDate = [DateTime]::ParseExact($User.LastLogonDate, "M/d/yyyy h:mm tt", $null)
                $DaysSinceLogon = (Get-Date) - $LastLogonDate
    ...
    if (-not $SkipSecurityTest) {
        ...
        & ".\Scripts\Get-SecurityReport.ps1" -DetailedReport
    } else {
        Write-Host ""
